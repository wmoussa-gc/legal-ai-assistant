        ...
        try:
            ...
            if self.scasp_path:
                ...
                if not result.get('success', False):
                    simplified_program = self._create_simplified_program(
                        program)
                    if simplified_program != program:
                        print(
                            f"s(CASP) failed with complex rules, trying simplified version...")
                        result = self._query_scasp(
                            simplified_program, query, timeout)
            ...
            # Determine if we actually succeeded
            has_answers = len(result.get('answers', [])) > 0
            actual_success = result.get('success', False) and has_answers
            ...
            if not actual_success:
                if result.get('error'):
                    ...
                    error_msg = 'Both s(CASP) and SWI-Prolog reasoning failed'
...
"""
    ...
    def _query_scasp(self, program: str, query: str, timeout: int) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = [self.scasp_path, '--human',
                   '--tree', '-s', '1', str(program_file)]
    ...
    def _parse_prolog_output(self, output: str) -> List[ScaspAnswer]:
        ...
        # Prolog returns empty output when query succeeds (exit code 0)
        # or contains 'true'/'yes' in interactive mode
        # If we got here, returncode was 0, so query succeeded
        answers.append(ScaspAnswer(
            solution={},
            justification=['Query succeeded - conditions are satisfied'],
            confidence=0.85,  # Good confidence for Prolog proof
            is_consistent=True
        ))
